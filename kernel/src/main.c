...
void finalizar_proceso(int pid_a_eliminar){
    ...
    if(!buscar_pid(pid_a_eliminar)){
        if(exec_global != NULL && exec_global -> contexto -> pid != pid_a_eliminar)
            ...
            send_interrupt(cpu_interrupt_fd);
...
void planificar_corto_plazo(){
    ...
    int algoritmo = elegir_algoritmo(algoritmo_planificacion);
    ...
    while(1){
        ...
        switch (algoritmo){
            ...
            case VRR:
                ...
                send_pcb(exec_global, cpu_dispatch_fd, EXEC_PCB);
                usleep(exec_global -> quantum * 1000);
                send_interrupt(cpu_interrupt_fd);
...
void logear_ready(t_list* lista){
    t_list_iterator* iterador = list_iterator_create(lista);
    t_pcb* paquete;
    int pids[list_size(lista)], contador = 0;
